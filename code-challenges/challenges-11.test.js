...
const toLastNames = people => {
  ...
  let names = people.map( obj => {
    namesArray.push(obj.firstName + " " + obj.lastName);
  });
  return namesArray;
};
...
const validateEmail = (email) => {
  ...
  var tester = new RegExp(regex);
  if (tester.test(email) && email.match(/\./) && /^[A-za-z]+/.test(email) && !/\.@/.test(email) && /\.+[A-Za-z]{3}$/.test(email) && !/^[a-zA-Z0-9]+\.[a-zA-Z0-9]+\.[a-zA-Z0-9]+@/.test(email)) {
    return true;
  } else {
    return false;
  }
...
const validatePhoneNumber = (phoneNumber) => {
  const regex1 = /^[1-9]\d{2}-\d{3}-\d{4}$/;
  const regex2 = /^\(\d{3}\)\s\d{3}-\d{4}$/;
  const regex3 = /^[1-9]\d{2}\s\d{3}\s\d{4}$/;
  const regex4 = /^[1-9]\d{2}\.\d{3}\.\d{4}$/;
  const regex5 = /^[1-9]\d{2}\s\d{3}-\d{4}$/;
  const regex6 = /^[1-9]\d{2}\s\d{3}\s\d{4}$/;
  const regex7 = /^[1-9]\d{2}\s\d{7}$/;
  const regex8 = /^[1-9]\d{9}$/;
  if (regex1.test(phoneNumber) || regex2.test(phoneNumber) || regex3.test(phoneNumber) || regex4.test(phoneNumber) || regex5.test(phoneNumber) || regex6.test(phoneNumber) || regex7.test(phoneNumber) || regex8.test(phoneNumber)){
    return true;
  } else {
    return false;
  }
