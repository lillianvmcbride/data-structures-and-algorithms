...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i <recipe.ingredients.length; i++) {
    holder[i] = (recipe.ingredients[i].trim().split(' '));
    if (holder[i].length === 3)
      result.push(holder[i][2]);
    else if (holder[i].length === 4)
      result.push(holder[i][2] + ' ' + holder[i][3]);
    else if (holder[i].length === 5)
      result.push(holder[i][2] + ' ' + holder[i][3] + ' ' + holder[i][4]);
  }
  console.log(result);
  return result;
};
...
const stepActions = (recipe) => {
  ...
  for (let i = 0; i <recipe.steps.length; i++) {
    holder[i] = (recipe.steps[i].trim().split(' '));
